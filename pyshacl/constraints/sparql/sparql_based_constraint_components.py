            ...
            for val, vio in violations:
                ...
                if isinstance(vio, bool):
                    if vio is False:  # ASKValidator Result
                        ...
                        rept = self.make_v_result(target_graph, f, value_node=val, **new_kwargs)
    ...
    def validate(
        ...
        for v in value_nodes:
            if query_helper is None:
                ...
                if "this" in init_binds and isinstance(focus, rdflib.BNode):
                    if executor.sparql_mode:
                        ...
                        raise ValidationFailure("Can only use BlankNode as target node when using RDFLib in-memory store.")
                if "value" in init_binds and isinstance(v, rdflib.BNode):
                    if executor.sparql_mode:
                        # If value node is a blank node, we technically can't use it in the sparql pre-binding.
                        raise ValidationFailure("Value Node cannot be a BlankNode, when running in SPARQL mode.")
                    elif not isinstance(target_graph.store, (Memory, SimpleMemory)):
                        raise ValidationFailure("Can only use BlankNode as ValueNode when using RDFLib in-memory store.")
            try:
...
class SelectConstraintValidator(SPARQLConstraintComponentValidator):
    ...
    def validate(self, executor, focus, value_nodes, target_graph, query_helper=None, new_bind_vals=None):
        ...
        if query_helper is None:
            ...
            # Value node does not get pre-bound in a SELECT Validator, only $this=focus.
            init_binds, sparql_text = query_helper.pre_bind_variables(
                focus, valuenode=None, extravars=bind_vals.keys()
            )
            sparql_text = query_helper.apply_prefixes(sparql_text)
            init_binds.update(bind_vals)
            if "this" in init_binds and isinstance(focus, rdflib.BNode):
        ...
        for r in results:
            var_dict = r.asdict()
            p = var_dict.get('path', None)
            v = var_dict.get('value', None)
            t = var_dict.get('this', None)
            f = var_dict.get('failure', None)
            if t is None and (f is None or f is False or (isinstance(f, rdflib.Literal) and not f.value)):
                raise ValidationFailure("Validation Failure generated by SELECTConstraintValidator.\n"
                                        "The SELECTConstraintValidator must return a '?this' or '?failure' in the SELECT vars.\n"
                                        "See https://www.w3.org/TR/shacl/#SPARQLSelectValidator")
            if f is True or (isinstance(f, rdflib.Literal) and f.value):
                raise ValidationFailure("Validation Failure generated by SELECTConstraintValidator.")
            else:
                violations.add((None, (t, p, v)))
